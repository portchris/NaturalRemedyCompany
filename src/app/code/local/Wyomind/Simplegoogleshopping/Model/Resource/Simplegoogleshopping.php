...
class Wyomind_Simplegoogleshopping_Model_Resource_Simplegoogleshopping extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function getTaxRates()
    {
        ...
        $select->joinleft(
            array('tc' => $tableTcc), 
            'tc.product_tax_class_id = ' . $tableTc . '.class_id', 'tc.tax_calculation_rate_id'
        );
        $select->joinleft(
            array('tcr' => $tableTcr), 
    ...
    public function getReviewsAndRates()
    {
        ...
        $sqlByStoreId->from(
            array("r" => $tableR), 
            array("COUNT(DISTINCT r.review_id) AS count", 'entity_pk_value')
        );
        ...
        $sqlAllStoreId->from(
            array("r" => $tableR), 
            array("COUNT(DISTINCT r.review_id) AS count", 'entity_pk_value', "(SELECT 0) AS  store_id")
        );
    ...
    public function getMediaGallery($storeId)
    {
        ...
        $select->joinleft(
            array('cpemgv' => $tableCpemgv),
            'cpemgv.value_id = ' . $tableCpemg . '.value_id', 
    ...
    public function getConfigurableProducts($storeId, $attributes)
    {
        ...
        $collection->getSelect()->joinLeft(
            $tableCpsl . ' AS cpsl', 'cpsl.parent_id=e.entity_id ', 
            array('child_ids' => 'GROUP_CONCAT( DISTINCT cpsl.product_id)')
        );
        $collection->getSelect()->joinLeft(
            $tableCsi . ' AS stock', 'stock.product_id=e.entity_id', 
    ...
    public function getConfigurableQuantity($storeId)
    {
        ...
        $collection->getSelect()->joinLeft(
            $tableCsi . ' AS stock', 
            'stock.product_id=cpsl.product_id', 
            array('qty' => 'SUM(stock.qty)')
        );
        $collection->getSelect()->group(array('cpsl.parent_id'));
    ...
    public function getGroupedProducts($storeId, $attributes)
    {
        ...
        $collection->getSelect()->joinLeft(
            ...
            array('child_ids' => 'GROUP_CONCAT( DISTINCT cpl.linked_product_id)')
        );
        $collection->getSelect()->joinLeft(
            $tableCsi . ' AS stock', 'stock.product_id=e.entity_id', 
            array('qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock',
                    'use_config_manage_stock' => 'use_config_manage_stock', 'backorders' => 'backorders', 
                    'use_config_backorders' => 'use_config_backorders')
        );
        $collection->getSelect()->joinLeft(
            $tableCur . ' AS url', 
            'url.product_id=e.entity_id ' . $this->notLike . ' AND is_system=1 '
                . 'AND ' . $this->options . ' AND url.store_id=' . $storeId, 
            array('request_path' => $this->concat . '(DISTINCT request_path)')
        );
        $collection->getSelect()->joinLeft($tableCcp . ' AS categories', 'categories.product_id=e.entity_id');
        $collection->getSelect()->joinLeft(
    ...
    public function getBundleProducts($storeId, $attributes)
    {
        ...
        $collection->getSelect()->joinLeft(
            $tableCpbs . ' AS cpbs', 
            'cpbs.parent_product_id=e.entity_id', 
            array('child_ids' => 'GROUP_CONCAT( DISTINCT cpbs.product_id)')
        );
        $collection->getSelect()->joinLeft(
            $tableCsi . ' AS stock', 
        ...
        $collection->getSelect()->joinLeft(
            $tableCur . ' AS url', 
            'url.product_id=e.entity_id ' . $this->notLike 
                . ' AND is_system=1 AND ' . $this->options . ' AND url.store_id=' . $storeId, 
            array('request_path' => $this->concat . '(DISTINCT request_path)')
        );
        $collection->getSelect()->joinLeft($tableCcp . ' AS categories', 'categories.product_id=e.entity_id');
        $collection->getSelect()->joinLeft(
    ...
    public function getCustomOptions()
    {
        ...
        $sqlCustomOptions->joinleft(
            array("cpot" => $tableCpot), 
            "cpot.option_id=cpo.option_id AND cpot.store_id=0", 
            array("option" => "title", "option_id", "store_id")
        );
        $sqlCustomOptions->joinleft(
            array("cpotv" => $tableCpotv), 
            "cpotv.option_id = cpo.option_id", 
            array("sku", "id" => "option_type_id")
        );
        $sqlCustomOptions->joinleft(
            array("cpott" => $tableCpott), 
            "cpott.option_type_id=cpotv.option_type_id AND cpott.store_id=cpot.store_id",
            "title AS value"
        );
        $sqlCustomOptions->joinleft(
            array("cpotp" => $tableCpotp), 
            "cpotp.option_type_id=cpotv.option_type_id AND cpotp.store_id=cpot.store_id", 
     ...
     */
    public function prepareProductCollection($storeId, $typeIdFilter, $visibilityFilter, $attributeSetsFilter, 
            $attributes, $attributesFilter, $categoriesFilterList, $categoryFilter, $categoryType
    )
    {
        $tableCsi = $this->resource->getTableName("cataloginventory_stock_item");
        $tableCur = $this->resource->getTableName("core_url_rewrite");
        ...
        if ($categoriesFilterList[0] != '*') {
            ...
            if (version_compare(Mage::getVersion(), '1.12.0', '<=')) {
                ...
                $collection->getSelect()->joinInner(
                    ...
                    '((categories_index.category_id=categories.category_id '
                        . 'AND categories_index.product_id=categories.product_id)) '
                        . 'AND categories_index.store_id=' . $storeId . ' ' . $filter, 
                    array('categories_ids' => 'GROUP_CONCAT( DISTINCT categories_index.category_id)')
                );
            } else {
                $filter = "AND categories.category_id " . $in . " (" . $filter . ")";
                $collection->getSelect()->joinInner(
                    $tableCcp . ' AS categories', $ct . ' ' . $filter, 
                    array('categories_ids' => 'GROUP_CONCAT( DISTINCT categories.category_id)')
                );
            }
        } else {
            $collection->getSelect()->joinLeft($tableCcp . ' AS categories', 'categories.product_id=e.entity_id');
        ...
        if (version_compare(Mage::getVersion(), '1.4.0', '>=')) {
            $collection->getSelect()->joinLeft(
                $tableCpip . ' AS price_index', 
                'price_index.entity_id=e.entity_id AND customer_group_id=0 '
                    . 'AND price_index.website_id=' . Mage::getModel('core/store')->load($storeId)->getWebsiteId(), 
                array('min_price' => 'min_price', 'max_price' => 'max_price', 
